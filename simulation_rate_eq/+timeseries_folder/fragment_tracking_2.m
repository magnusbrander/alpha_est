...
function [fragments] = fragment_tracking_2(posLenImage,ionicStrength)

% FUNCTION: [fragments] = fragment_tracking_2(posLenImage)
% Decription:
% Input: posLenImage = a barcode image with center of mass positions
% weighted with the length of each fragment
% Output: A series of fragments and their time evolution 
...
nrOfScores = 7;
stateScore = zeros(size(im,1),nrOfScores);
...
for time = 2:size(posLenImage,1)
    ...
    while frag <= tempNumOfFrag
        ...
        if prevState == 1
            ...
            % Check for split case if previous is non positive
            if tempScore(1) == 1
                % If the fragment is found intact we simply update the
                % fragment sequence           
                fragUpdate = [currFragPositions(matchIndex),currFragLengths(matchIndex),...
                    prevState,fragmentID,0,matchIndex,time];
