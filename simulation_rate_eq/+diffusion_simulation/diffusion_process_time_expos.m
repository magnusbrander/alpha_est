...
function fragmentEvolution = diffusion_process_time_expos(cutInfo,maxTime,timeResolution,intialLength,pixelSize,diffusionConst,salt)
% ########################################################################
%
% FUNCTION: fragmentEvolution = diffusion_process(cutInfo,intialLength,pixelSize,diffusionConst) 
% Description: Here we implement the Gillespie algorithm for simulation of
% the DNA diffusion process taking into acount the finite resolution of the
% experimental recording contributing to discrete and time averagedcut
...
% Discretize time into elements equal to the time resolution to mimic the
% experiments and save memory

timeIntervals = timeResolution:timeResolution:maxTime;

% Make sure we have at least one time interval 

if isempty(timeIntervals)
    timeIntervals = timeResolution;
end



% Create cell array for storing all positions at all time intervals
fragmentEvolution = cell(1,length(timeIntervals));
...
for i = 1:length(timeIntervals)
    ...
    % Loop through all times where something happens
    ...
    fragmentEvolution{i}{1} = fragPos;
    fragmentEvolution{i}{2} = theTime;
