function [cuttingInformation,timeToN0] = nicking_process_time(L,frayDist,...
    maxTime,alpha,initialNickNr)
...
while time < maxTime
    ...
    % By construction we now know that the outcome is either a new nick or
    ...
    if foundCut
        ...
        % Increment the cut count variable
        cutCount = cutCount + 1;
        % Save the position of the cut
        cutPositions(1,cutCount) = cutPosRow;
        % Save the fragment number at which the cut happened
        cutAtFragNr(1,cutCount) = fragToNick;
    ...
    if sum(nrNonNicked) == 0
        fprintf('All sites have been nicked')
...
cuttingInformation.numberOfCuts = cutCount;
